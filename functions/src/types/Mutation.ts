...
export const Mutation = mutationType({
	definition(t) {
		t.field('createUser', {
			...
			resolve: (_parent, { data }) => <any>createUser(data),
		});
		t.field('login', {
			type: User,
			args: {
			...
			resolve: (_parent, { username, password }) => login(username, password),
		});
		t.field('createTodoListItemByUser', {
			...
			resolve: (_parent, { data, userId }) => <any>createTodoListItemByUser(data, userId),
		});
	},
});
